...
def main():
    ...
    X_train_combined = np.concatenate([X_train_labeled, X_train_labeled_optical_flow])
    y_train_combined = np.concatenate([y_train_labeled, y_train_labeled_optical_flow ])
    ...
    # create log dir
    if not os.path.exists("./logs/optical"):
        os.makedirs("./logs/optical")

    current_time = str(int(time.time()))

    # csv logs based on the time
    csv_logger = CSVLogger('./logs/optical/log_' + current_time + 'total.csv', append=True, separator=';')
    ...
    checkpointer = ModelCheckpoint(filepath='./logs/optical/best_model_' + current_time + 'total.hdf5', verbose=1,
    ...
    for epoch_idx in range(epochs):
        ...
        pseudo_labels = (pseudo_labels + pseudo_labels_optical_flow)/2
        del pseudo_labels_optical_flow
        gc.collect()
        ...
        if len(pseudo_dataset) > 0:
            ...
            if len(y_values) >= unlabelled_batch_size:
                pseudo_data_strong_aug = DataGenerator(x_values, None, y_pseudo, unlabelled_batch_size, True, 10, 3, 10)
    ...
    fig.savefig('./logs/optical/plot' + current_time + '.svg', format='svg')

    model.load_weights('./logs/optical/best_model_' + current_time + 'temporal.hdf5')
