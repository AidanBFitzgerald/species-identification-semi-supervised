...
def main():
    ...
    epochs = 200
    batch_size = 32
    unlabelled_batch_size = batch_size * 4
    learning_rate = 0.01
    threshold = 0.8
    ...
    checkpointer = ModelCheckpoint(filepath='./logs/I3D/best_model_' + current_time + '.hdf5', verbose=1,
    ...
    for epoch_idx in range(epochs):
        ...
        learning_rate = base_learning_rate * 0.5 * (math.cos(((epoch_idx + 1)/epochs) * math.pi) + 1)
        print(learning_rate)
        model.compile(loss='categorical_crossentropy', optimizer=SGD(lr=learning_rate), metrics=["accuracy"])
